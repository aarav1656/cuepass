...
Priya, a localization and accessibility QC lead at an indie distributor, ships a
finished restoration. Four weeks later it comes back rejected on caption reading speed.
Hardcoded subtitle QC tools bake in one constant (`MAX_CPS = 17`) and never notice that
Netflix publishes two adult limits on two live pages. Cuepass cannot bake that in:
**Parallel Search** finds each buyer's candidate pages and **Parallel Extract** opens
them; without Parallel credentials the run raises `ParallelUnavailableError` and there
is no cached-constants mode. Measured number: **61** cues of *The Iron Mask* clear the
20 cps page and fail the 17 cps page. Honest limit: Cuepass never rewrites subtitle
text.
...
The frustrating part for Priya is not that somebody measured wrong. It is that "the Netflix limit" is not a single fact.
...
**Parallel Search then Parallel Extract: both load-bearing, and the split between them is the product.**

A Search-only research brief would paste snippets into Gemini and call it done. Cuepass
refuses that shape. Search returns candidate URLs and nothing else. Extract opens the
page, carries the Search `session_id`, and only then may Python read a threshold out of
the returned text. Through the official `parallel-web` SDK (1.3.3), `import parallel`,
exposed as two `google.adk.tools.FunctionTool` objects that every desk holds.
